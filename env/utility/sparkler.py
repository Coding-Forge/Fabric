...
class Sparkler:
    ...
    def to_redshift(self,
        ...
        """
        Load Sparkler Dataframe as a Table on Amazon Redshift.
        :return: None
        """
        ...
        spark: SparkSession = self._session.spark_session
        casts: Dict[str, str] = Sparkler._extract_casts(dataframe.dtypes)
        dataframe = Sparkler.date2timestamp(dataframe)
        dataframe.cache()
        ...
        try:
            ...
            if num_files_returned != num_partitions:
                raise Exception(f"{num_files_returned} files returned. {num_partitions} expected.")
